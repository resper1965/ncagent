...
import { AgentPersona } from '../../lib/agent-system'
...
export default function AskPage() {
  ...
  const [selectedAgents, setSelectedAgents] = useState<AgentPersona[]>([])
  ...
  const handleSingleAgentSelect = (agent: AgentPersona | null) => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-zinc-950 via-zinc-900 to-zinc-950">
      ...
      {/* Chat Container */}
      <div className="container mx-auto px-6 py-6 h-[calc(100vh-120px)]">
        <div className="max-w-4xl mx-auto h-full flex flex-col">
          ...
          <div className="mb-4 flex items-center justify-between">
            ...
            <div className="flex-1 relative">
              <button
                ...
                className="w-full bg-zinc-800 border border-zinc-700 rounded-lg p-4 flex items-center justify-between hover:bg-zinc-700 transition-colors"
              >
                <div className="flex items-center space-x-3">
                  ...
                  <div className="text-left">
                    <p className="text-sm text-zinc-300">Selected Agent(s)</p>
                    <p className="font-medium text-white">
                      {getSelectedAgentsText()}
                    </p>
                  </div>
                </div>
                <ChevronDown className={`h-5 w-5 text-zinc-400 transition-transform ${showAgentSelector ? 'rotate-180' : ''}`} />
              </button>
                  ...
                  <div className="p-2">
                    ...
                    <div className="p-3 border-b border-zinc-700">
                      <label className="flex items-center space-x-2">
                        <input
                          ...
                          className="rounded border-zinc-600 bg-zinc-700 text-primary focus:ring-primary"
                        />
                        <span className="text-sm text-zinc-300">Enable conversational memory</span>
                      </label>
                    </div>
                      ...
                      <div className="p-3 border-b border-zinc-700">
                        <label className="flex items-center space-x-2">
                          <input
                            ...
                            className="rounded border-zinc-600 bg-zinc-700 text-primary focus:ring-primary"
                          />
                          <span className="text-sm text-zinc-300">Enable debate between agents</span>
                        </label>
                      </div>
                    )}
                    ...
                    >
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          <p className="font-medium text-white">n.Agent Default</p>
                          <p className="text-sm text-zinc-300">General assistant for documents</p>
                        </div>
                      </div>
                    </button>
                    ...
                    <div className="mt-2">
                      <p className="text-xs text-zinc-400 px-3 py-1">Single Agent:</p>
                      {agents.map((agent) => (
                        <button
                          key={agent.id}
                          onClick={() => {
                        ...
                        >
                          <div className="flex items-center space-x-3">
                            ...
                            <div>
                              ...
                              <p className="text-sm text-zinc-300">{agent.title}</p>
                            </div>
                          </div>
                        </button>
                      ))}
                    ...
                    <div className="mt-2">
                      <p className="text-xs text-zinc-400 px-3 py-1">Multiple Agents (Debate):</p>
                      {agents.map((agent) => (
                        <button
                          key={`multi-${agent.id}`}
          ...
          {showSessionInfo && currentSessionId && (
            <div className="mb-4 p-4 bg-zinc-800 border border-zinc-700 rounded-lg">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-zinc-300">Active Session</p>
                  <p className="text-xs text-zinc-400 font-mono">{currentSessionId}</p>
          ...
          {/* Messages */}
          <div className="flex-1 bg-zinc-800 rounded-lg shadow-sm border border-zinc-700 overflow-hidden">
            <div className="h-full flex flex-col">
              {/* Messages Area */}
              <div className="flex-1 overflow-y-auto p-6 space-y-4">
                {messages.length === 0 ? (
                  <div className="text-center py-12">
                    <Bot className="mx-auto h-12 w-12 text-zinc-400 mb-4" />
                        ...
                        <div className="flex items-start space-x-2">
                          {message.role === 'assistant' && (
                            <div className="flex items-center space-x-2">
                              ...
                              ) : message.agent_used && message.agent_used !== 'n.Agent Default' ? (
                                <Brain className="h-5 w-5 text-primary mt-0.5 flex-shrink-0" />
                              ) : (
                                <Bot className="h-5 w-5 text-zinc-500 mt-0.5 flex-shrink-0" />
                              )}
                              {message.agent_used && (
                            ...
                            {message.type === 'multi_agent' && message.multi_agent_responses && (
                              <div className="mt-4 space-y-3">
                                <div className="border-t border-zinc-600 pt-3">
                                  <h4 className="text-sm font-medium text-zinc-300 mb-2">Individual Responses:</h4>
                                  {message.multi_agent_responses.map((response, index) => (
                                    <div key={index} className="mb-3 p-3 bg-zinc-600 rounded-lg">
                                      <div className="flex items-center space-x-2 mb-2">
                                        <Brain className="h-4 w-4 text-primary" />
                                        <span className="text-sm font-medium text-white">{response.agent_used}</span>
                                      </div>
                                      <p className="text-sm text-zinc-200">{response.content}</p>
                                    </div>
                                  ))}
                                </div>
                                ...
                                {message.consensus && (
                                  <div className="border-t border-zinc-600 pt-3">
                                    ...
                                    <p className="text-sm text-zinc-300">{message.consensus}</p>
                                  </div>
                                )}
                              ...
                              <div className="mt-3 pt-3 border-t border-zinc-600">
                                ...
                                <div className="space-y-1">
                                  {message.sources.slice(0, 3).map((source, index) => (
                                    <div key={index} className="text-xs text-zinc-300 bg-zinc-600 p-2 rounded">
                                      {source.content}
                                    </div>
                                  ))}
                                </div>
                ...
                <div className="flex space-x-4">
                  <div className="flex-1">
                    <textarea
                      ...
                      className="w-full p-3 bg-zinc-700 border border-zinc-600 rounded-lg resize-none focus:ring-2 focus:ring-primary focus:border-transparent text-white placeholder-zinc-400"
                      rows={2}
                      disabled={loading}
                    />
                  </div>
