...
export default function AgentEditModal({ isOpen, onClose, onSuccess, agent }: AgentEditModalProps) {
  ...
  useEffect(() => {
    if (agent) {
      setFormData({
        ...
        focus: agent.focus || '',
        icon: agent.icon || '🤖',
        core_principles: agent.core_principles?.length > 0 ? agent.core_principles : [''],
        expertise_areas: agent.expertise_areas?.length > 0 ? agent.expertise_areas : [''],
        communication_style: agent.communication_style || 'Professional and clear',
        response_format: agent.response_format || 'Structured with examples'
      })
    }
  }, [agent])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        alert('Error updating agent: ' + data.message)
      }
    } catch (error) {
      ...
      <div className="bg-slate-800 border border-slate-700 rounded-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div className="p-6 border-b border-slate-700">
          <div className="flex items-center justify-between">
            <h2 className="text-xl font-bold text-white">Edit Agent: {agent.name}</h2>
            <button
              onClick={onClose}
